...
import { ScanFace, Cpu } from "lucide-react";
...
export function HeroSection({ onSpecs, onSim }: HeroSectionProps) {
  ...
  return (
    <section
      ref={ref}
      className="relative h-[120vh] w-full overflow-hidden bg-[#050510] perspective-1000"
    >
      {/* Layer 1 */}
      <motion.div style={{ y: gridY }} className="absolute inset-0 z-0 opacity-30">
        <div
      ...
      {/* Layer 2 */}
      <motion.div style={{ y: droneY }} className="absolute left-[10%] top-[20%] z-10 hidden lg:block">
      ...
      {/* Layer 3 */}
      <motion.div style={{ y: headsetY }} className="absolute right-[5%] top-[15%] z-20 hidden lg:block">
      ...
      {/* Main content */}
      <motion.div style={{ y: textY, opacity: textOpacity }} className="relative z-30 flex h-full flex-col items-center justify-center px-4 text-center">
        ...
        <div className="flex flex-col gap-4 sm:flex-row">
          <button
            onClick={onSim}
            className="group relative overflow-hidden rounded-lg bg-white px-8 py-4 font-bold text-black transition-transform hover:scale-105"
          >
